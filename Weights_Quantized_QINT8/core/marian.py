...
class MarianOnnx(GenerationMixin):
    def __init__(self, path: str, device: str = 'cpu'):
        ...
        self.final_logits_weight = torch.load(os.path.join(path, 'lm_weight.bin'))
        if self.final_logits_weight.dtype == torch.qint8:
            print("Dequantizing qint8 embedding weights...")
            self.final_logits_weight = self.final_logits_weight.dequantize()
        self.final_logits_weight = self.final_logits_weight.to(self.device)
    ...
    def greedy_search(self, input_ids, encoder_output, attention_mask):
        ...
        max_length = self.config.max_length or 128  # or any reasonable default
