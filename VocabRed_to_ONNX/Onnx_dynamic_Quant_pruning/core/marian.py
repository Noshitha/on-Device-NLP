...
#         max_length = self.config.max_length or 128  
...
class MarianOnnx(GenerationMixin):
    ...
    def _init_sequence_length_for_generation(self, input_ids, max_length: int):
        unfinished_sequences = torch.zeros(input_ids.shape[0], dtype=torch.int8, device=input_ids.device) + 1
        sequence_lengths       = torch.zeros(input_ids.shape[0], dtype=torch.int8, device=input_ids.device) + max_length
        cur_len = input_ids.shape[-1]
        return sequence_lengths, unfinished_sequences, cur_len
    ...
    def _encoder_forward(self, input_ids, attention_mask):
        ...
        last_hidden_state = self.encoder_session.run(None, inputs)[0]
        return torch.from_numpy(last_hidden_state).to(input_ids.device)
    ...
    def _prepare_decoder_input_ids_for_generation(self, input_ids, decoder_start_token_id: int):
        decoder_input_ids = torch.ones(
            (input_ids.shape[0], 1), dtype=torch.int64, device=input_ids.device
        ) * decoder_start_token_id
        return decoder_input_ids
    ...
    def generate(self, input_ids, attention_mask):
        decoder_start_token_id = self.config.decoder_start_token_id
        encoder_output = self._encoder_forward(input_ids, attention_mask)
        input_ids = self._prepare_decoder_input_ids_for_generation(input_ids, decoder_start_token_id)
        return self.greedy_search(input_ids, encoder_output, attention_mask)