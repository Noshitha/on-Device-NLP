...
1) Loads MarianMT model
2) Extracts and wraps encoder/decoder for export
3) Saves shared weights (lm_weight.bin, lm_bias.bin)
4) Traces encoder & decoder using dummy input and exports to encoder.onnx / decoder.onnx
"""
...
def create_marian_encoder_decoder(model_path: str, outdir: str):
    """Create a MarianMTModel encoder & decoder and export shared weights and tokenizer artifacts."""
    ...
    # Files that might be present (from various tokenizer formats)
    possible_files = [
        'config.json',
...
def generate_onnx_graph(model_path, tokenizer_path, encoder_path, decoder_path, outdir, quant=True):
