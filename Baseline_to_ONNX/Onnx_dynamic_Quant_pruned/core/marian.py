...
Handles generate() logic using ONNX Runtime
Implements greedy decoding and final logit computation
"""
...
class MarianOnnx(GenerationMixin):
    ...
    def _decoder_forward(self, input_ids, encoder_output, attention_mask):
        ...
        inputs = {
            ...
            "attention_mask":             attention_mask.cpu().numpy(),
            # feed back the shared embedding we pruned out:
            #"decoder.embed_tokens.weight": self.final_logits_weight.cpu().numpy(),
            "decoder.embed_tokens.weight": self.final_logits_weight.detach().cpu().numpy(),
        }
    ...
    def generate(self, input_ids, attention_mask, max_length=None):
        # default to config if caller didn’t pass one
        if max_length is None:
            max_length = self.config.max_length or 50

        # encoder
        encoder_output = self._encoder_forward(input_ids, attention_mask)

        # start with just the BOS/decoder_start token
        ...
        # pass max_length through
        return self.greedy_search(input_ids, encoder_output, attention_mask, max_length)

    def greedy_search(self, input_ids, encoder_output, attention_mask, max_length: int):
        pad_token_id = self.config.pad_token_id
        eos_token_id = self.config.eos_token_id

        # now your helper uses the passed max_length
        ...
        while cur_len < max_length:
            ...
            # force‐eos propagation
            input_ids[input_ids[:, -2] == eos_token_id, -1] = eos_token_id

            unfinished_sequences = unfinished_sequences.mul((next_tokens != eos_token_id).to(torch.int8))
