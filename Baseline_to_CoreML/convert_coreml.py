...
from core.utils_coreml import generate_coreml_graph

PARAMS = None

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("input", type=str, help="Hugging Face model identifier, e.g., 'Helsinki-NLP/opus-mt-fr-en'")
...
def main():
    ...
    local_model_dir = os.path.join("./local_models", model_name.replace('/', '_'))
    ...
    # encoder_path = os.path.join(outdir, "encoder.mlmodel")
    # decoder_path = os.path.join(outdir, "decoder.mlmodel")

    encoder_path = os.path.join(outdir, "encoder.mlpackage")
    decoder_path = os.path.join(outdir, "decoder.mlpackage")

    generate_coreml_graph(local_model_dir, encoder_path, decoder_path, outdir)
